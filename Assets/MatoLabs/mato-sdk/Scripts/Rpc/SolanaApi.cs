...
namespace SolanaWalletAdapter.Scripts
{
    public static class SolanaApi
    {
        ...
        private static async Task<T> MakeRequest<T>(JsonRpcRequest requestData) where T: class
        {
            ...
            using (var request = new UnityWebRequest(PublisherSettings.I.ClusterUrl, "POST"))
            {
        ...
        public static async Task<List<TokenAccountData>> GetTokenAccounts(string pubkey)
        {
            return await GetTokenAccountsByOwnerWithProgramId(pubkey, SOLANA_TOKEN_PROGRAM);
        }
